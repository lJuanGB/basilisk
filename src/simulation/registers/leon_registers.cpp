...
LeonRegisters::LeonRegisters()
{
    ...
    this->BlackLionConnectionInfo = "None";
    return;
}
...
void LeonRegisters::SelfInit()
{
    ...
    RegisterWriter* Snorkel_RWSpeeds = new RegisterWriter(this->rwSpeedsMsgName_fswIn, this->rwSpeedsMsgID_fswIn, sizeof(RWSpeedIntMsg));
    ...
    RegisterWriter* Snorkel_AttNav = new RegisterWriter(this->simpleNavAttMsgName_fswIn, this->simpleNavAttMsgID_fswIn, sizeof(NavAttIntMsg));
    
    /* Add snorkel-writers to the Qemu interface*/
    QemuSingleton::GetInstance()->add_snorkel_writer(Snorkel_RWSpeeds);
    QemuSingleton::GetInstance()->add_snorkel_writer(Snorkel_AttNav);
...
void LeonRegisters::update_registers_from_fsw()
{
    ...
    {
        uint8_t *local_buffer = new uint8_t[(*it)->msg_size];
        SystemMessaging::GetInstance()->ReadMessage((*it)->msg_id, &LocalHeader, (*it)->msg_size, local_buffer, this->moduleID);
        (*it)->create_packet_from_FSWdata(local_buffer);
        delete [] local_buffer;
    }
    return;
}
