...
class CppWrapperClass(object):
        ...
        for i_task in self.taskIdxDir:
            ...
            for model in task.TaskModels:
                ...
                for k, v in addressDict.items():
                    ...
                    # dictUse = eval('self.' + methodType + '_dict')
                    # modelID = self.findAddressMatch(v, modelTag, dictUse)
    ...
    def classify_list(self, list, prefix):
        # @TODO: check of list is all zeros. In that case we don't swig anything, srry.
        len_list = len(list)
...
class MpyWrapCodeTemplate(object):
    ...
    def initialize_code(self, model_tag, algs_dict, hpp_line):
        ...
        # '%s.Def < SysModelFunctionNames::setModelTag > (& SysModel::setModelTag);\n' % self.wrap_name + \
        # '%s.Def < SysModelFunctionNames::getModelTag > (& SysModel::getModelTag);\n' % self.wrap_name + \
    ...
    def add_properties_to_wrap(self, current_vars_swig_dict):
        str_props = ''
        # str_props = '%s.Property("%s", &%s::%s, &%s::%s);\n' % (self.wrap_name, "ModelTag", self.class_name, values[0],
